...
async def test_user():
    ...
    yield user
    # Cleanup: attempt to delete the test user, catch and ignore connection closure errors
    try:
        async with writable_session() as session:
            await session.execute(text(f"DELETE FROM users WHERE email = '{user.email}'"))
            await session.commit()
    except Exception as cleanup_error:
        # Check if the error message indicates a closed event loop or missing send method
